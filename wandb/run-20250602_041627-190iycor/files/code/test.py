...
class ModelTester:
    def __init__(self, config_path="config/config.yaml", model_path=None, test_data_path=None):
        ...
        if model_path:
            ...
            if os.path.exists(best_model_path):
                ...
                print("🏆 Using best_model for testing")
            elif os.path.exists(final_model_path):
                self.trained_model_path = final_model_path
                print("📁 Using final_model for testing")
            else:
    ...
    def load_test_data(self):
        ...
        if max_samples > 0 and len(df) > max_samples:
            ...
            print(f"✅ Loaded {len(df)} test samples from {test_csv} (limited by max_val_samples: {max_samples})")
        else:
    ...
    def load_base_model(self):
        print("🔄 Loading base PaliGemma model...")
    ...
    def load_trained_model(self):
        print("🔄 Loading TSN-integrated trained model...")
        ...
        try:
            ...
            print("✅ Loaded processor from trained model")
        except Exception:
    ...
    def calculate_loss(self, model, processor, test_data, model_name):
        ...
        print(f"📊 Calculating loss on ALL {total_samples} samples")

        # Process in batches for better performance and memory efficiency
        batch_size = 4  # Conservative batch size for loss calculation
                ...
                try:
                    ...
                    for _, row in batch_df.iterrows():
                        ...
                        try:
                            ...
                            # For suffix method, labels are handled automatically
                    ...
                    print(f"⚠️  Batch {batch_idx} processing error: {str(e)[:50]}...")
                    continue

        avg_loss = total_loss / max(valid_samples, 1) if valid_samples > 0 else 0.0
        print(f"✅ {model_name} - Average Loss: {avg_loss:.4f} (Valid samples: {valid_samples}/{total_samples})")
    ...
    def generate_captions(self, model, processor, test_data, model_name):
        ...
        # Process ALL test images, not just a sample
        test_sample = test_data
        total_samples = len(test_sample)
        print(f"📝 Processing ALL {total_samples} test images")
        ...
        with torch.no_grad():
            for _, row in tqdm(test_sample.iterrows(), total=len(test_sample), desc=f"Captions - {model_name}"):
                ...
                try:
                    ...
                    try:
                        # Method 1: Standard format with proper prompt
                        inputs = processor(
                            images=image,
                            text=prompt_text,
                            return_tensors="pt",
                        ...
                        try:
                            # Method 2: With explicit padding
                            inputs = processor(
                                images=image,
                                text=prompt_text,
                                return_tensors="pt",
                            ...
                            # Method 3: Fallback to CPU processing
                            inputs = processor(
                                images=image,
                                text=prompt_text,
                                return_tensors="pt",
                    ...
                    # Generate with safer parameters and token validation
                    ...
                    with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                        try:
                            ...
                            if hasattr(model, 'paligemma'):
                                # TSN model - use underlying PaliGemma
                    ...
                    try:
                        ...
                        # Clean up the generated text - remove prompt and formatting
                    ...
                    print(f"❌ Error generating caption: {str(e)[:100]}...")
                    generated_captions.append("")
                    ground_truths.append(caption)
        ...
        print(f"✅ Generated {successful_gens}/{len(generated_captions)} successful captions from {total_samples} samples")
    ...
    def calculate_metrics(self, generated_captions, ground_truths):
        ...
        print("📊 Calculating BLEU, METEOR, ROUGE-L scores...")
        ...
        print("📊 Calculating CIDEr and SPICE scores...")
    ...
    def calculate_spice_score(self, generated_captions, ground_truths):
        try:
            ...
            print(f"⚠️  SPICE calculation failed: {e}")
            return 0.0
    ...
    def save_comparison_results(self, base_captions, tsn_captions, ground_truths, test_data, base_metrics, tsn_metrics):
        ...
        print("🔄 Calculating individual metrics for base model...")
        base_individual = self.calculate_individual_metrics(base_captions, ground_truths)

        print("🔄 Calculating individual metrics for TSN model...")
    ...
    def create_improvement_chart(self, base_metrics, tsn_metrics):
        ...
        print(f"✅ Improvement chart saved: {chart_path}")
        return chart_path

    def run_test(self):
        print("🚀 Starting Base vs TSN model comparison test...")
        print("="*60)
